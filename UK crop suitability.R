...
#uri = paste("http://rest.soilgrids.org/soilgrids/v2.0/properties/query?lon=",lon,"&lat=",lat,"&property=phh2o&property=sand&property=silt&property=clay&depth=0-5cm&depth=0-30cm&depth=5-15cm&depth=15-30cm&depth=30-60cm&depth=60-100cm&depth=100-200cm&value=mean", sep="")
...
#out <- as_tibble(as.data.frame(plyr::rbind.fill(out)))
#out <- out %>% select(starts_with("layer.name"),starts_with("layers.depths.mean."))
#out
...
#mydb = dbConnect(MySQL(), user='root', password='yuna2UtR', dbname='cropbase_5_0_3', host='127.0.0.1')

# Running Queries:
#   Queries can be run using the dbSendQuery function.
# rs = dbSendQuery(mydb, 'select c.id, c.name,
...
#                  b.season_length_min,b.season_length_max,
#                  
...
#                  
#                  from 
#                  ag_agroecology a, ag_season b , crop_records c 
#                  
#                  where 
#                  a.cropid=b.cropid and b.cropid=c.id 
#                  
...
for (i in nrow(grid)){
  ...
  if (any(is.nan(utemp))){
    ...
    clim_suit_norain <- cbind(CROP_ID = ecology[k,2],TCnorain, 
                              AvgMonths = mean(as.numeric(TCnorain)))
    ...
    avgTotSoilSuit=cbind.data.frame( cropid = ecology$id[i] , averagesoilsuit, name = ecology$name[i])
  ...
  #}
    ...
    climSoil$avgclimsoil <- mean(climSoil$AvgMonths, climSoil$averagesoilsuit)
  ...
  topcrop = rbind(topcrop,
                  climSoil_ID_crops[which(climSoil_ID_crops$avgclimsoil == max(climSoil_ID_crops$avgclimsoil))[1],])
